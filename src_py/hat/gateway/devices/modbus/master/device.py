...
class ModbusMasterDevice(aio.Resource):
    ...
    async def _connection_loop(self):
        ...
        try:
            self._log.debug('starting connection loop')
            ...
            while True:
                ...
                try:
                    self._conn = await aio.wait_for(
                        connect(self._conf['connection'], self._conf['name']),
                    ...
                    self._log.info('connecting error: %s', e, exc_info=e)
                    await self._set_status('DISCONNECTED')
                    await asyncio.sleep(
                ...
                self._log.debug('creating remote devices')
    ...
    def _enable_remote_device(self, device_id):
        ...
        if not device:
            self._log.debug('device %s is not available', device_id)
            return
        if not device.conn.is_open:
            self._log.debug('connection is not available')
            return

        reader = self._readers.get(device_id)
        if reader and reader.is_open:
            self._log.debug('reader %s is already running', device_id)
            return

        self._log.debug('creating reader for device %s', device_id)
    ...
    async def _write(self, device_id, data_name, request_id, value):
        ...
        if response:
            self._log.debug('writing result: %s', response.result)
